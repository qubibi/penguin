class Nyu {
	constructor() {
		...
		this.is_utatteta = false;
		this.kyouseilock = kyouseilock
	}
	...
	f_mouseup() {
		...
		if ( Math.random()< 0.2 ) this.pressspd = qb.rndm(-0.08,-0.18);
		...
		if (this.is_utatteta) {
			...
			koukaon.setVolume(random(0.0, 0.6));
			koukaon.play();
			this.is_utatteta = false;
	...
	f_ef() {
		...
		if ( Math.random()< 0.077 )  ootoa.koegatogireruyo();
...
this.openclosev_tgt = 0.33*this.pressv+qb.mppng(this.kasanmy, 0, 333, 0, 0.66);
   ...
   tojitaaitacheck(v, t, cb) {
	...
	f_kutigahiraita() {
		this.is_utatteta = true;
		zawakend.f_fornyu_kutigaaita()
