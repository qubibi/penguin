class KLGLOTT88Oscillator {
	...
	updateParameters() {
	  ...
	  this.a = 27 * this.currentSq / (4 * this.nk * sqSum * sqSum)+Math.random()*0.0001;
...
class SnMusic {
	constructor() {
		...
		this.vowelAttackTime = 0.15; // Time to reach maximum volume (seconds)
		this.vowelReleaseTime = 0.1; // Time to reach zero volume (seconds)
		this.vowelF1min = 300;
		this.vowelF1max = 1900;
		this.vowelF2min = 2000;
		this.vowelF2max = 800;
		this.vowelF3min = 1000;
		this.vowelF3max = 2200;
		...
		this.pitchSmoothingFactor = .55; // Value between 0 and 1. Smaller is smoother currentpitch determination
...
//     this.monoReverb = new MonoReverb(this.audioContext);
//     this.vowelGain.disconnect();
//     this.vowelGain.connect(this.monoReverb.input);
//     this.monoReverb.connect(this.audioContext.destination);
	...
	adjust(mx, my) {
		...
		// Control vowelFiltf1 frequency with Y-axis position (without using margin)
			this.vowelTargetF1 = map(globalOpenclosev, 0.42, 1, this.vowelF1min, this.vowelF1max);
		...
		this.vowelTargetF2 = map(globalOpenclosev, 0.42, 1, this.vowelF2min, this.vowelF2max);
		// Update F3
	...
	playVowel() {	
		...
		if ( Math.random()< 0.4 ) this.vibratoStartTime = millis() / 2000;
		this.vibratoDepth = random(5, 13.6);
	...
	updateBackgroundVolume() {
		...
		this.backgroundGain.amp(adjustedVolume*musiconryomag);
	}
	...
	updateMusicVolume() {
		...
		if (this.musicGain) {
			this.musicGain.amp((this.musicVolume*slowmusicmag) *musiconryomag);
		}
		if (this.music) {
			// this.music.rate(this.musicPitchMultiplier);
		}
