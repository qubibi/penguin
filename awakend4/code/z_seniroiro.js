class Z_seniroiro {
	...
	f_posset() {
		...
		this.tweset_futaeuwakuti = [
			...
			{ frm:900,    scx:1.07, scy:1.7, r:-5, x:3, y:-0 },
			{ frm:1000,    scx:1.11, scy:1.75, r:-5, x:3, y:-10 }
		]
		this.tw_futaeuwakuti = qt.generateTweenArrays(this.tweset_futaeuwakuti)
		this.tweset_kumasitakuti = [
			{ frm:0, scx:0.95, scy:0.8, r:5, x:-9, y:23 },
			{ frm:330, scx:0.95, scy:0.8, r:5, x:-9, y:23 },
			{ frm:666,    scx:0.9, scy:-0.2, r:0, x:0, y:8 },
			{ frm:750,    scx:0.9, scy:0.6, r:0, x:0, y:53 },
			{ frm:800,    scx:1.12, scy:1.7, r:-8, x:-6, y:23 },
			{ frm:1000,    scx:1.12, scy:1.7, r:-1, x:0, y:0 }
		]
	...
	f_efdrawoutside(_g) {
		...
		let _mag5 = qb.mppng(nnyu.openclosev,0.33 ,0.77,  0,1)
		...
		qline.drawline(_g, _kumasita, 2.1-_mag4*0.7, 2.5, true, "#000000bb")
...
let wave = (sin(frameCount/(10 + sin(frameCount/50)*1)) + sin(frameCount/(25 + sin(frameCount/60)*1.3))) * 0.5;
		...
		let aaa = qvert.shiftScaleXY(this.vt_matugeroot, 1.4,1.4,0, -30+(30*wave), -14*_mag )
		...
		qline.drawline(_g, mmm, 15+15*_mag4, 10, true, this.tw_namidayodare[nnyu.openclosev1000].col)
	...
	f_efdrawinside(_g) {
		// sen medama
		let _mag3 = qb.mppng(nnyu.openclosev,0.5 ,0.6, 1, .7)
